...
The contrast sensitivity \\(C\\) is determined by pixel bias currents, which set the speed and threshold voltages of the change detector, and are generated by an on-chip digitally programmed bias generator. Typical [DVS Cameras](#dvs-cameras) can set the threshold to 10%-50% illumination change.
...
filters and [Spiking Neural Networks]({{<relref "spiking_neurons_lit_review.md#" >}}). Filters and SNNs fuse incoming events
...
**[Voxel Grid]({{<relref "voxel_grid.md#" >}})** is a space-time (3D) histogram of events, where each voxel
