...
\begin{equation}
  \left\{ X_1, \dots, X_{i-1} \right\} = \mathrm{Pa}_{X_i} \in \mathbb{Z}
\end{equation}

where $\mathbb{Z} \in \mathrm{ND}_{X_i}$. Form the local
independencies for $X_i$ and from the position property it follows
that $X_i \perp \mathbb{Z} | \mathrm{Pa}_{X_i}$. Hence $P(X_i| X_1,
...
\rightleftharpoons \dots \rightleftharpoons X_n$. Let $\matbb{Z}$ be a
...
- Whenever we have a v-structure $X_{i-1} \rightarrow X_i \leftarrow
  X_{i+1}$, then $X_i$ or one of its descendants are in $\mathbb{Z}$;
...
$Y \in \mathbb{Y} $ given $\mathbb{Z}$. We use
