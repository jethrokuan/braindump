...
localization problems (in comparison to [[file:ekf_localization.org][§ekf_localization]] and
[[file:markov_localization.org][§markov_localization]]).
...
MC localization uses the particle filter ([[file:particle_filter.org][§particle_filter]]) to
