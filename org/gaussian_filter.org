...
([[file:bayes_filter.org][§bayes_filter]]) for continuous spaces.
...
Using the moments representation gives rise to the Kalman Filter ([[file:kalman_filter.org][§kalman_filter]]).
