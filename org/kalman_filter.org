...
1. The Markovian Assumption ([[file:markovian_assumption.org][§markovian_assumption]])
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State ${K}_t = \hat{\Sigma}_t C_t^T (C_t \hat{\Sigma}_t C_t^T + Q_t)^{-1}$
...
Kalman Filter ([[file:extended_kalman_filter.org][§extended_kalman_filter]]).

* Related
[[file:gaussian_filter.org][- §gaussian_filter]]
