...
In model-free [[file:reinforcement_learning.org][Reinforcement Learning ⭐]], we assumed that $p(s_{t+1} |
...
wrong, for example by using [[file:gaussian_processes.org][Gaussian Processes]].
