...
A Hopfield network is a fully connected [[file:ising_models.org][Ising model]] with a symmetric
...
\begin{equation}
  p(y_s = 1 | \mathbb{y_{-s}}, \mathbb{\theta}) =
  \textrm{sigm}(\mathbb{w_{s,:}}^T y_{-s} + b_s)
\end{equation}
