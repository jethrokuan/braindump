...
Standard [[file:imitation_learning.org][§imitation_learning]] copies actions performed by the expert,
...
1. Given $\psi$, compute [[file:control_as_inference.org::*Backward Messages][backward message]] $\beta(s_t, a_t)$
2. Given $\psi$, compute [[file:control_as_inference.org::*Forward Messages][forward message]] $\alpha(s_t)$
