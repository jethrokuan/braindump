...
1. Super class is more /general/ than sub class
2. Applying inheritance on group of similar classes can result in common
   parts among classes being extracted into more general classes
3. Multiple inheritance not allowed in Java, but allowed in C++

** Class level members
   :PROPERTIES:
   :CUSTOM_ID: class-level-members
   :END:

1. Static variables
2. Accessed by class name rather than class instance
...
1. Association that represents a strong whole-part relationship
2. Implemented using a solid diamond on side of parent
3. No cyclical links, i.e. no association to itself
...
1. Container-contained relationship, i.e. if one gets deleted the other
   still functions
2. Implemented with a blank diamond
3. e.g. Initialises an object it needs by taking it in as an argument
   rather than instantiating it within the class
...
1. Methods with different signatures(take in different type and/or
   number of variables)
...
1. Dashed arrows show dependencies

   - Weaker association as compared to composition and aggregations (not
     long-term i.e. no object is kept as they are only received as
     parameters)

2. Constraints given within curly braces
3. Asssociations can be shown as an attribute instead of a life
...
* Diagram Legends
  :PROPERTIES:
  :CUSTOM_ID: diagram-legends
  :END:

** Object diagrams:
   :PROPERTIES:
   :CUSTOM_ID: object-diagrams-1
   :END:

[[https://nus-cs2103.github.io/website/book/oopDesign/associations/basic/images/completeStructure.png]]
 

1. Associations - normal lines (e.g. between AgeList and Main)

** Class diagrams:
   :PROPERTIES:
   :CUSTOM_ID: class-diagrams-1
   :END:

[[https://nus-cs2103.github.io/website/book/uml/classDiagrams/introduction/what/images/classDiagramExample1.png]]
 

1.  Associations - Normal lines with arrows (e.g. between ModelManager
    and Phase)
2.  Association labels - Arrows with text (e.g. Phase <- is for PR)
3.  Association roles - e.g. Tutor is mentor
4.  Multiplicity - e.g. 1-3 Teams consisting of 3-5 Students
5.  Bi-directional associations - Arrows both ways
6.  Composition (Whole part relationship i.e. everything is destroyed
    together) - Solid black diamond
7.  Aggregation (container-contained relationship) - Blank diamond
8.  Dependencies(Weaker associations) - Dashed arrows
9.  UML notes - Boxes
10. Constraints - Notes within curly braces
11. Associations - Attributes (name : type [multiplicity] = default
    value) (Multiplicity and default values are blank)
12. Abstract classes - (abstract) above Class name
13. Association classes (Either draw out from association or create the
    class in between the 2 associated classes)

** Sequence diagrams:
   :PROPERTIES:
   :CUSTOM_ID: sequence-diagrams-2
   :END:

1.  Method calls - Solid arrows
2.  Method returns (Can be omitted) - Dotted arrows
3.  No underlined object names (e.g. :Object)
4.  Loops - labelled boxes
5.  Activation bar (Can be omitted)
...
6.  Deletion - Use a X at end of lifeline of an object
7.  Self Invocation - Draw a second bar within the activation bar for
    inner method and an arrow to show self invocation
8.  Alternative paths - =Alt= frames (boxes) with dotted horizontal
    lines to separate alternative paths
9.  Optional paths - =Opt= frames
10. Reference frames
...
11. Parallel paths - For multi-threading, as multiple things are being
    done at the same time

** Architecture diagrams:
   :PROPERTIES:
   :CUSTOM_ID: architecture-diagrams-1
   :END:

1. Free-form, no UML specifications
2. Avoid bi-directional arrows, as there can be loss of information
   e.g. bi-directional dependency

** Activity diagrams
   :PROPERTIES:
   :CUSTOM_ID: activity-diagrams-1
   :END:

1. Actions - Rectangles with rounded edges (Steps)
2. Control flows : Lines with arrowheads (Flow of control from one
   action to another)
3. Alternate paths - Diamond shapes

   - Branch or merge nodes
   - Each control flow leaving branch node has guard condition
   - Only 1 alernative path can be taken at any time.

4. Parallel paths - Bar
